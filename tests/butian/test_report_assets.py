...
class ReportAssetTests(unittest.TestCase):
    ...
    def test_current_risk_table_rejects_malicious_security_ids(self):
        data = {
            "generated_at": "2026-06-05 09:05:50",
            "project": {"name": "demo", "path": "/tmp/demo", "ecosystems": ["npm"]},
            "scan_config": {"scan_mode": "full_dependency_scan"},
    ...
    def test_html_report_assets_ship_secret_evidence_click_and_edit_handlers(self):
        with open(REPORT_JS, "r", encoding="utf-8") as handle:
            js = handle.read()

        self.assertIn("function editSecretEvidence(button)", js)
        self.assertIn("function editorUrlForFile(path, line)", js)
        self.assertIn("function fallbackEditCommand(path)", js)
        ...
        self.assertIn("secret-edit-btn", js)
        self.assertIn(">编辑</button>", js)
        self.assertNotIn("secret-copy-btn", js)
        self.assertNotIn("function copySecretEvidence", js)
        self.assertNotIn("button.textContent", js)
    ...
    def test_html_renders_secret_code_context_with_line_numbers(self):
        ...
        self.assertIn('class="secret-edit-btn"', html)
        self.assertIn('onclick="event.stopPropagation();editSecretEvidence(this)"', html)
        self.assertIn(">编辑</button>", html)
        self.assertIn('data-file=".env.example"', html)
        self.assertIn('data-line="17"', html)
        self.assertNotIn('class="secret-copy-btn"', html)
    ...
    def test_html_renders_secret_code_context_in_yellow_card(self):
        ...
        self.assertIn('class="secret-edit-btn"', html[evidence_pos:])
        self.assertIn('onclick="event.stopPropagation();editSecretEvidence(this)"', html[evidence_pos:])
        self.assertIn(">编辑</button>", html[evidence_pos:])
        self.assertIn('data-file=".env.example"', html[evidence_pos:])
        self.assertIn('data-line="17"', html[evidence_pos:])
        self.assertNotIn('class="secret-copy-btn"', html[evidence_pos:])
