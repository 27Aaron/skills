...
| 2   | 仓库安检报告   | 渲染硬编码密钥、敏感文件、gitignore 状态、GitHub Actions、依赖配置与维护、IaC/容器结构化分组 |
| 3   | 过期依赖报告   | 渲染版本维护信号表格                                                                          |
...
3. **仓库安检** — 硬编码密钥、敏感文件、gitignore 状态、GitHub Actions 工作流安全、依赖配置与维护、IaC/容器配置
...
- **本地规则分组**：新增的 GitHub Actions、依赖配置与维护、IaC/容器 finding 以中文分组展示，保留依据和处理方式，便于专业用户复核
