...
| 可利用性 | 勒索利用优先显示 `勒索利用`；KEV 显示 `已知利用`；否则有 EPSS 时显示百分比 |
| 发现时间 | `cve_enrichments[].nvdPublishedAt` 的日期；无数据显示 `-`                  |

Markdown 只保留修复决策最常用的字段和简短利用信号，不展开 HTML 里的详情、长 tooltip、攻击条件和代码级互动。
