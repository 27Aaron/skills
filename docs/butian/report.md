...
| #   | 职责           | 说明                                                                                                        |
| --- | -------------- | ----------------------------------------------------------------------------------------------------------- |
| 1   | 风险项表格渲染 | 将风险项列表渲染为 Markdown 表格（含严重度、包名、版本、CVE/GHSA、修复版本、EPSS/CVSS 等简短信号）          |
...
| `render_vulnerabilities(analysis)` | 风险项表格（严重度、包名、版本、CVE/GHSA、修复版本、说明；说明内追加 EPSS/CVSS/CWE/KEV/NVD）    |
...
| `enrichment_summary(item)`               | 将增强信号压缩成表格内短文本，如 `EPSS 91.8%；CVSS 8.6；CWE-918`          |
...
Markdown 报告面向小白用户和后续修复流程，默认保持简洁，不再额外生成编号式重复清单。依赖漏洞的修复关键信息集中在“命中风险项”表格中：

- 包名、当前版本、修复版本用于直接定位升级目标。
- CVE/GHSA 保持 Markdown 链接，方便追溯公告来源。
- EPSS/CVSS/CWE/CISA KEV/NVD 作为短标签追加在“说明”列，帮助判断优先级，但不展开长段解释。

示例表格行：

```markdown
| 高风险 | next | 16.2.4 | [CVE-2026-44578](https://www.cve.org/CVERecord?id=CVE-2026-44578) | 16.2.5 | 服务端请求伪造风险；EPSS 91.8%；CVSS 8.6；CWE-918；NVD 2026-05-13 |
```

HTML 报告继续承载更完整的交互详情、tooltip 和可视化复核信息；Markdown 只保留修复时最常用的字段。

## 设计要点

- **模板驱动**：报告结构由 `templates/report.md` 控制，修改模板即可调整输出格式
- **中文标签**：所有面向用户的文本使用中文严重度标签
- **安全编号链接化**：Markdown 中的 CVE/GHSA 会生成可点击链接，方便用户追溯来源
- **风险信号压缩**：EPSS/CVSS/CWE/KEV/NVD 被压缩进风险表格说明列，避免报告尾部重复生成长上下文
