...
| 2   | 仓库安检报告   | 渲染硬编码密钥、敏感文件、gitignore 状态、GitHub Actions、仓库治理/供应链、IaC/容器结构化分组 |
| 3   | 过期依赖报告   | 渲染版本维护信号表格                                                                          |
...
| `render_hygiene(analysis)`         | 硬编码密钥统计 + 表格、敏感文件统计 + 表格、gitignore 状态、本地仓库安检分组表格 |
...
3. **仓库安检** — 硬编码密钥、敏感文件、gitignore 状态、GitHub Actions 工作流安全、仓库治理/供应链配置、IaC/容器配置
...
| `gitignore_missing` | `.gitignore`                | 摘要说明缺失规则，并在人工确认项里提示补充             |
| `workflow_checks`   | `GitHub Actions 工作流安全` | 表格展示等级、位置、检查项、证据、建议                 |
| `repository_checks` | `仓库治理与供应链配置`      | 表格展示等级、位置、检查项、证据、建议                 |
| `iac_checks`        | `IaC / 容器 / 部署配置`     | 表格展示等级、位置、检查项、证据、建议                 |
...
空状态遵循保守表达：没有任何 hygiene 数据时只写"没有发现硬编码密钥、被 git 跟踪的敏感文件或缺失的敏感文件忽略规则"，不表达"绝对安全"。结构化本地规则的低风险和 info 项也会保留，方便专业用户看到仓库治理、发布完整性和供应链维护信号。
...
- **本地规则分组**：新增的 GitHub Actions、仓库治理/供应链、IaC/容器 finding 以中文分组展示，保留证据和建议，便于专业用户复核
