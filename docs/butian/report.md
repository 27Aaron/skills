...
```bash
python3 report.py .butian/<timestamp>/assets/analysis.json                   # 自动输出到 docs/
python3 report.py analysis.json docs/security-report-2025-01-15.md           # 指定输出路径
```
...
| `output_markdown` | 位置参数 | ❌   | 输出 Markdown 路径（默认 `<project>/docs/security-report-<date>.md`） |
...
默认路径规则：`<project_path>/docs/security-report-<YYYY-MM-DD>.md`
