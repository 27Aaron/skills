...
`server_inventory.py` 将 `server_collect.py` 采集到的原始输出标准化为服务器资产，包括 Linux 发行版、系统包、当前内核、监听端口、OpenSSH 登录配置、防火墙状态和 Docker 容器元数据。
...
| 5   | 登录与防火墙 | 解析 OpenSSH 关键配置和 ufw/firewalld/nftables/iptables 状态     |
