...
| 4   | HTML 输出       | 生成 `.butian/<run>/content/security-report.html`               |
| 5   | 浏览器打开控制  | 首次扫描自动打开；复扫或 `--no-open` 时跳过                     |
...
| `output_html`   | 位置参数 | 否   | 输出 HTML 路径，默认 `.butian/<run>/content/security-report.html` |
| `--no-open`     | flag     | 否   | 不自动打开浏览器                                                  |
...
| `open_decision(args, output_path)`   | 根据 `--no-open`、环境变量和 `.first-scan-done` 判断是否打开 |
| `skipped_open_message(reason)`       | 输出跳过自动打开的中文原因                                   |
| `_first_scan_done(output_path)`      | 检查 `.butian/.first-scan-done`                              |
| `_mark_first_scan_done(output_path)` | 创建 `.butian/.first-scan-done`                              |
...
| 安全编号 | CVE 链接到 cve.org，GHSA/其他链接到 OSV           |
| 修复版本 | 用 chip 列表展示，多个版本换行                    |
...
命中行附近的空行也会保留。例如命中第 17 行时，可以展示 15-19 行，帮助用户回到源文件定位。
...
首次成功打开后写入 `.butian/.first-scan-done`。后续复扫默认不再自动打开浏览器；`--no-open` 或 `BUTIAN_NO_OPEN=1` 会明确打印跳过原因。
