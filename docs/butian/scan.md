...
- **密钥预览策略**：`secret_preview()` 默认对硬编码密钥做预览化展示；`generic_*`、连接串、私钥等高风险格式会继续脱敏，但会保留变量名和值的首尾片段，避免报告里只剩 `***` 而无法定位。
...
- **代码证据窗口**：`code_context` 最多保留 3 行；命中首行时展示文件开头 3 行，命中末行时展示文件末尾 3 行，中间命中时保留前后各 1 行。
