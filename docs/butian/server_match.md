...
`server_match.py` 负责把 Linux 服务器系统包和可匹配的内核包提交到官方漏洞源查询，并构造证据闭环的服务器风险项。
...
- Debian/Ubuntu 二进制包如果带 `source_name`，优先使用 source package 查询，例如 `libssl3t64` 使用 `openssl`，内核安装包使用 `linux`。
