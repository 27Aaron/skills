...
| 4   | 仓库安检归一化 | 将硬编码密钥、敏感文件、GitHub Actions、依赖与发布治理、IaC/容器 finding 转成统一行动项 |
...
`SECRET_TYPE_LABELS` 将 `aws_access_key`、`private_key`、`slack_token` 等技术标识映射为中文标签（如"AWS 访问密钥"、"私钥"）。
