...
- **修复要你点头同意。** 报告生成并打开后，Agent 先用 AskUserQuestion 询问是否修复（确认修复 / 取消修复）；确认后再用 AskUserQuestion 询问升级策略（升级到已知修复版本 / 全部升级到最新版本）。硬编码凭证占位符、Dependabot 配置、过期依赖更新等待确认维护动作也必须用 AskUserQuestion 单独确认。Agent 不会自行执行任何升级命令或创建治理配置。
...
1. **硬编码凭证占位符**：如果报告中有 `疑似硬编码凭证`，尤其是 `.env.example`、`.env.sample`、`.env.template`、`.env.dist` 等模板文件命中，必须用 AskUserQuestion 询问是否处理。推荐问题：`发现 N 个硬编码凭证占位符/疑似凭证，需要我帮你逐项确认并处理吗？` 选项为 `处理凭证占位符` / `暂不处理`。用户确认后，先让用户判断是否真实可用；如只是模板占位符，替换为明显不可用的示例值；如是真实可用凭证，先轮换或撤销，再移除明文。
2. **创建 Dependabot 配置**：如果报告的 `仓库安检 / 依赖配置与维护` 中出现 `配置 Dependabot`，说明本地检测到 GitHub remote 和 GitHub Dependabot 官方支持的生态，但仓库缺少 `.github/dependabot.yml`。必须用 AskUserQuestion 询问是否创建。推荐问题：`检测到仓库托管在 GitHub，是否创建 .github/dependabot.yml 来开启依赖更新 PR？` 选项为 `创建 Dependabot 配置` / `暂不创建`。用户确认后才运行：
...
3. **更新过期依赖**：如果 analysis 或 HTML 报告中 `outdated` / `过期依赖` 数量大于 0，且本轮尚未执行过 `--strategy latest`，必须用 AskUserQuestion 询问是否更新。推荐问题：`发现 N 个过期依赖，是否按维护窗口更新到最新版本？` 选项为 `更新过期依赖` / `暂不更新`。用户确认后运行：
...
- 用户选择 `确认修复`、`将依赖升级到已修复版本`、`全部依赖升级到最新版本`、`升级父依赖并重新扫描` 或 `强制覆盖残留依赖` 时，继续原本修复流程，不弹出待确认动作队列。
- 用户选择 `取消修复`、`暂不处理`、`暂不验证`，或依赖修复流程准备生成最终报告前，如果待确认动作队列非空，必须先用 AskUserQuestion 逐项询问上面三类动作；用户都处理或都跳过后，再生成最终报告/结束。
- 如果用户已经选择 `全部依赖升级到最新版本` 或已运行 `fix.py --strategy latest`，视为已经处理过期依赖维护，不再重复弹出 `更新过期依赖`。
- 典型场景：提示 `顶层依赖已升级成功；残留 3 个中风险项来自父依赖锁定的嵌套旧版本。是否继续升级父依赖并重新扫描？` 时，如果用户选择 `升级父依赖并重新扫描`，正常执行 parent-upgrade，不弹出待确认动作队列；如果用户选择 `暂不处理`，先弹出硬编码凭证占位符、创建 Dependabot 配置和更新过期依赖这三个 AskUserQuestion，然后再进入最终报告。

#### 第一轮：顶层依赖升级

1. **是否修复**：用 AskUserQuestion 提供 `确认修复` / `取消修复`。提问时用"风险项"替代"依赖漏洞"，格式如"发现 N 个风险项（X高/Y中/Z低），是否执行修复？"（数字与级别之间无空格）。用户选择取消时，停止依赖修复；如待确认动作队列非空，先逐项询问，再总结报告路径和主要风险。
2. **修复方式**：用户确认修复后，用 AskUserQuestion 提供 `将依赖升级到已修复版本` / `全部依赖升级到最新版本`。提问时写"选择升级策略：升级到已知修复版本还是全部升级到最新版本呢？"
   ...
   - **全部依赖升级到最新版本**：升级到每个包的最新大版本（可能跨大版本，风险较高）
...
6. **是否继续**：用 AskUserQuestion 提供 `升级父依赖并重新扫描` / `暂不处理`。用户选择 `升级父依赖并重新扫描` 时，正常继续下一步，不弹出待确认动作队列。用户选择暂不处理时，先处理待确认动作队列，再生成最终报告并结束。
7. **执行升级**：用户选择继续后，运行：
...
10. **是否继续**：用 AskUserQuestion 提供 `强制覆盖残留依赖` / `暂不处理`。用户选择 `强制覆盖残留依赖` 时，正常继续下一步，不弹出待确认动作队列。用户选择暂不处理时，先处理待确认动作队列，再生成最终报告并结束。
11. **执行覆盖**：用户选择继续后，运行：
