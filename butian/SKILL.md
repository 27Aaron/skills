...
- **修复要你点头同意。** 报告生成并打开后，Agent 先用 AskUserQuestion 询问是否修复（确认修复 / 取消修复）；确认后再用 AskUserQuestion 询问升级策略（升级到已知修复版本 / 全部升级到最新版本）。硬编码凭证占位符、Dependabot 配置等仓库安检待确认动作也必须用 AskUserQuestion 单独确认。Agent 不会自行执行任何升级命令或创建治理配置。
...
依赖漏洞修复之外，报告里可能还有需要用户点头的仓库安检动作。Agent 先把它们放入待确认动作队列，不要在依赖修复继续进行时打断用户。
...
- 用户选择 `取消修复`、`暂不处理`、`暂不验证`，或依赖修复流程准备生成最终报告前，如果待确认动作队列非空，必须先用 AskUserQuestion 逐项询问上面两类动作；用户都处理或都跳过后，再生成最终报告/结束。
- 典型场景：提示 `顶层依赖已升级成功；残留 3 个中风险项来自父依赖锁定的嵌套旧版本。是否继续升级父依赖并重新扫描？` 时，如果用户选择 `升级父依赖并重新扫描`，正常执行 parent-upgrade，不弹出待确认动作队列；如果用户选择 `暂不处理`，先弹出硬编码凭证占位符和创建 Dependabot 配置这两个 AskUserQuestion，然后再进入最终报告。
