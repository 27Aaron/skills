...
const CWE_PLAIN = {
  ...
  "CWE-79": "存在页面脚本注入风险，攻击者可能在网页中植入恶意代码，影响访问用户的数据安全",
  ...
  "CWE-90": "存在 LDAP 注入风险，攻击者可能绕过身份验证或获取目录服务中的敏感数据",
  ...
  "CWE-113": "存在 HTTP 响应头注入风险，攻击者可能篡改响应内容或设置恶意 Cookie",
  ...
  "CWE-134": "存在格式化字符串漏洞，攻击者可能读取内存中的敏感数据或导致程序崩溃",
  ...
  "CWE-208": "存在时序攻击风险，攻击者可能通过对比响应时间推测出加密密钥等敏感信息",
  "CWE-209": "存在错误信息泄露敏感数据的问题，攻击者可能从报错信息中获取系统内部细节",
  ...
  "CWE-377": "存在临时文件处理不安全的问题，攻击者可能读取或替换临时文件中的数据",
  ...
  "CWE-434": "存在文件上传不限制类型的问题，攻击者可能上传恶意脚本并在服务器上执行",
  ...
  "CWE-494": "存在未校验下载内容完整性的问题，攻击者可能在传输过程中替换为恶意代码",
  ...
  "CWE-502": "存在反序列化漏洞，攻击者可能通过构造恶意数据在服务器上执行任意代码",
  ...
  "CWE-565": "存在依赖 Cookie 但未做完整性校验的问题，攻击者可能篡改 Cookie 内容",
  ...
  "CWE-943": "存在跨站请求伪造风险（使用 Ajax），攻击者可能通过构造请求冒充用户",
  ...
  "CWE-1325": "存在信任边界管理不当的问题，来自不可信来源的数据被当作可信数据处理",
...
function vulnerabilityExplanation(r) {
  ...
  return esc(
    `当前版本${advisorySummaryText(r)}；${shortFixedVersionText(r)}`,
  );
}
