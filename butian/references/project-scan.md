...
- `.butian/cache/`
- `docs/butian/<日期>/security-report.md`
- `docs/butian/<日期>/security-report.html`
- `docs/butian/<日期>/security-report-final.md`
- `docs/butian/<日期>/security-report-final.html`
- `.gitignore` 中用于忽略 `.butian/` 和上述生成报告文件的规则

这些条目描述的是项目内写入位置。只要把报告路径展示给用户，无论是终端摘要、修复前后的转述还是最终回复，都只展示 HTML/Markdown 报告的完整绝对路径，不能改写成 `docs/butian/...` 相对路径，也不要展示内部 `analysis.json` 路径。
...
- **过期依赖**：默认不扫描系统包和全局包，也不执行项目内包管理器或本地虚拟环境；只有显式传入 `--allow-project-exec` 时，才允许过期检查执行 `npm` / `pnpm` / `yarn` / `uv` / `.venv/bin/python` / `go` / `cargo` 等项目工具。
...
IaC/容器本地规则属于仓库安检，只能发现配置层明显缺口，不等于镜像漏洞扫描或 SBOM 扫描。
...
`analyze.py` 会生成确定性 `analysis.json`：`schema_version`、漏洞排序、`risk_summary`、`summary`、`red/yellow/green`、仓库安检项、过期依赖和扫描错误。后续只允许做轻量复核和业务语言润色；不要删除已确认风险项，不要把过期依赖改写成漏洞，不要把脱敏预览扩展成完整密钥。
...
生成报告时先写 HTML，再写 Markdown。Markdown 报告按以下顺序组织：报告总结、当前风险、仓库安检、过期依赖、覆盖说明、扫描错误、下一步建议。

项目 HTML 报告阅读流：项目概览、报告总结、仓库安检、当前风险、过期依赖、优先处理、待确认事项、扫描错误。静态 HTML 文件写入 `docs/butian/<日期>/security-report.html`；最终复扫写入 `docs/butian/<日期>/security-report-final.html`。对用户展示时必须使用对应文件的绝对路径。
...
- 每次 AskUserQuestion 只能确认一个阶段问题；即使接口支持一次传入多个问题，也必须只传 1 个问题。
- 禁止把“是否修复 / 修复方式”和“可选收尾动作”放进同一个 AskUserQuestion；用户界面中不应同时出现“修复范围”和“收尾动作”两个 tab。
- AskUserQuestion 的问题 header 只能使用：`修复确认`、`修复方式`、`收尾动作`、`残留依赖`、`强制覆盖`、`运行验证`。禁止使用 `长期维护`、`安全策略` 等未定义 header。
- 选项必须来自本文件对应阶段的白名单；禁止临时添加 `README 记录安全策略`、`开启长期维护` 等未定义选项。要新增选项，必须先更新本文件和测试。
...
依赖漏洞修复之外，报告里可能还有需要确认的可选收尾动作。它们不是“修复范围”，也不是“长期维护”开关。先把它们放入可选收尾动作队列，不要在依赖修复继续进行时打断当前流程，更不能和 `是否修复` 或 `修复方式` 放进同一个 AskUserQuestion。
...
如果队列非空，在依赖修复、残留处理和复扫都完成后，生成最终报告前，单独使用一个 header 为 `收尾动作` 的多选 AskUserQuestion。提问文案：

```text
下面是报告中发现的可选收尾动作。它们不是本轮依赖漏洞修复的必要步骤，可以跳过。要现在处理哪些？
```

底部的 `跳过收尾动作` 是互斥选项。用户选择后不执行其他收尾动作，直接进入最终报告。
...
- 用户选择 `开始修复`、`升级到修复版本`、`全部升级到最新版`、`继续处理`（升级父依赖并重新扫描）或 `强制覆盖` 时，继续原本修复流程，不弹出可选收尾动作队列。
- 用户选择 `先不修复` 时，不弹出可选收尾动作队列；只保留报告并结束。后续如果用户点名要求处理某个收尾动作，再单独确认。
- 用户选择 `暂不处理`（残留依赖或强制覆盖阶段）时，不弹出可选收尾动作队列；直接生成最终报告或结束。
- 依赖修复流程准备生成最终报告前，如果可选收尾动作队列非空，必须先用一个单独的多选 AskUserQuestion 询问白名单动作；用户选择处理所选动作或选择 `跳过收尾动作` 后，再生成最终报告。
- 最终报告生成后的 `运行验证` / `暂不验证` 只处理验证，不得再触发可选收尾动作队列。
- 如果用户已经选择 `全部升级到最新版` 或已运行 `fix.py --strategy latest`，视为已经处理过期依赖维护，不再重复弹出 `更新过期依赖`。

不要回退到旧式逐项弹窗；可选收尾动作统一进入一个独立多选问题。不要把它命名为 `长期维护`。
...
`--final-report` 会重新生成 `security-report-final.md` 和 `security-report-final.html`。报告不会自动打开，只在终端展示绝对路径和摘要。
