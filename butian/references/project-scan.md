...
`analyze.py` 会生成确定性 `analysis.json`：漏洞排序、`risk_summary`、`summary`、`red/yellow/green`、仓库安检项、过期依赖和扫描错误。Agent 之后只能做轻量复核和业务语言润色；不要删除已确认风险项，不要把过期依赖改写成漏洞，不要把脱敏预览扩展成完整密钥。

- 命中风险项：所有风险项按影响程度排序，全部放入 `top_issues`，不要只放前 5 个。必须透传 `advisory_id`、`aliases`、`cve_id`、`package`、`version`、`severity`、`summary`、`fixed_versions` 等字段。
...
Markdown 报告按以下顺序组织：报告总结、服务器运行环境、命中风险项、仓库安检、过期依赖、覆盖说明、扫描错误、下一步建议。未启用服务器扫描时明确写“未启用服务器运行环境扫描”。

项目 HTML 报告阅读流：项目概览、报告总结、服务器运行环境、仓库安检、命中风险项、过期依赖、优先处理、待确认事项、扫描错误。静态 HTML 文件路径为 `.butian/<timestamp>/content/security-report.html`。

命中风险项和过期依赖都默认展示 7 条，数量更多时用只读展开/收起按钮查看剩余全部条目；表格列宽必须稳定，修复版本列不能挤压或覆盖 GHSA 列。
