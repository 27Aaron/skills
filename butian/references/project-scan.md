...
Markdown 报告按以下顺序组织：报告总结、当前风险、仓库安检、过期依赖、覆盖说明、扫描错误、下一步建议。

项目 HTML 报告阅读流：项目概览、报告总结、仓库安检、当前风险、过期依赖、优先处理、待确认事项、扫描错误。静态 HTML 文件路径为 `docs/butian/<日期>/security-report.html`；最终复扫为 `docs/butian/<日期>/security-report-final.html`。
...
`--final-report` 会重新生成 `security-report-final.md` 和 `security-report-final.html`。报告不会自动打开，只在终端展示路径和摘要。
