...
HTML 报告源码拆分为 `templates/report.html`、`static/report.css` 和 `static/report.js`，`build_report.py` 把 CSS/JS 内联进最终的 `security-report.html`，最终报告是一个可单独移动和双击打开的 HTML 文件。Markdown 报告源码为 `templates/report.md`，`render_markdown.py` 用 `string.Template` 填充动态内容。
