...
CAPABILITY_BOUNDARY = (
    ...
    "此 Skill 会帮助你发现应用依赖漏洞、过期依赖和仓库暴露风险，"
    "并在显式提供 SSH 目标或离线 inventory 时做 Linux 服务器运行环境只读检查；"
    "SSH 采集必须使用密钥登录，脚本会禁用密码和键盘交互回退。"
    "应用依赖漏洞查询只处理本地可确认精确包名和精确版本的应用依赖坐标；"
    "服务器漏洞只处理能关联到发行版包坐标、内核包或官方包管理器安全更新的证据，"
    "不把 Dockerfile、compose、Kubernetes、devcontainer、镜像/SBOM、未关联服务 banner、"
    "自编译二进制或模糊 CPE 混入已确认漏洞，也不能替代代码审计、渗透测试或完整部署安全评估；"
